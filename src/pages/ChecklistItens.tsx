...
type EstoqueItem = Pick<Database["public"]["Tables"]["itens_estoque"]["Row"], "id" | "nome" | "sku">;
...
const ChecklistItens = () => {
  ...
  const loadInitial = async () => {
    try {
      await Promise.all([loadItems(), loadChecklists(), loadEstoque()]);
    } catch (error) {
  ...
  const loadEstoque = async () => {
    const { data } = await supabase.from("itens_estoque").select("id, nome, sku");
  ...
  const loadItems = async () => {
    const { data, error } = await supabase
      ...
      .select(
        ...
        item:itens_estoque ( id, nome, sku )
      `
      )
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload: ChecklistItemInsert = {
      ...
      item_id: formData.item_id === "none" ? null : formData.item_id,
    };
      ...
      <div className="space-y-6 p-4 md:p-6">
        <div className="flex flex-col gap-3 rounded-lg border bg-card p-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Cadastre itens vinculados a checklists e estoque.</p>
          </div>
          ...
          <Card className="lg:col-span-1">
            ...
            <CardContent>
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="space-y-2">
                  <Label>Item de estoque (opcional)</Label>
                  <Select
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Item estoque</TableHead>
